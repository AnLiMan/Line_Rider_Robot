...
int DeltaT = 300; // Переменная частоты снятия показаний с энкодеров (миллисекунды)
...
void LineRider() {
  ...
  byte InfraredSensorStraightState = digitalRead(InfraredSensorStraight); // 0 - белый, чёрный - 1

  // Если датчик посередине на черной линии
  if (InfraredSensorStraightState == 1) {
    analogWrite(LeftMotor, idleSpeed);
    analogWrite(RightMotor, int (idleSpeed * RightMotorAdj));
  }
